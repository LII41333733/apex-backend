...
public class BaseTradeManager {
    ...
    public TradeRecord<BaseTrade> watch(TradeMap baseTradeMap) throws IOException, URISyntaxException {
        logger.debug("BaseTradeManager.watch: Start");
            ...
            for (TradeLegMap.Entry<Long, TradeLegMap> baseOrderEntry : baseTradeMap.entrySet()) {
                ...
                if (tradeOpt.isPresent()) {
                    ...
                    if (hasFillOrder) {
                        ...
                        if (trade.getStatus().ordinal() < CANCELED.ordinal()) {
                            ...
                            if (trade.isOpen() || trade.hasRunners()) {
                                if (trade.getStatus().ordinal() > PENDING.ordinal()) {
                                    logger.debug("BaseTradeManager.watch: Updating Stops and Trims: {}", id);

                                    if (lastPrice <= trade.getStopPrice()) {
                                        logger.info("BaseTradeManager.watch: Stop Hit!: {}", id);
                                        baseTradeService.placeMarketSell(trade, STOP);
                                        filledTrades.add(id);
                                        trade.setStatus(FILLED);
                                        logger.info("BaseTradeManager.watch: (OPEN/RUNNERS -> FILLED): {}", id);
                                    } else {
                                        openTrades.add(id);
                                    ...
                                    if (trade.isOpen()) {
                                        ...
                                        if (trade.getTrimStatus() > 1) {
                                            runnerTrades.add(id);
                                            logger.info("BaseTradeManager.watch: Last Price: {}", lastPrice);
                                            logger.info("BaseTradeManager.watch: Last Price > Stop Price: {}", lastPrice > trade.getStopPrice());
                                ...
                                if (!trade.isFinalized()) {
                                    ...
                                    logger.info("BaseTradeManager.watch: (FILLED -> FINALIZED): {}", id);
                                }
                            }
                        }
                        ...
                        logger.debug("BaseTradeManager.watch: Finish: Saving Trade to Repo");
                    } else {
                        logger.error("BaseTradeManager.watch: Trade exists but no FILL order available. ID: {}", id);
                    }
                }
            }
        }
